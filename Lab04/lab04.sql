...
#Insertion of elements into Table countries:
...
#Insertion into of Table departments:
...
#Insertion into Table employees:
...
('112', 'JoseManuel', 'Urman', 'JMURMAN', '515.124.4469', '1987-06-29', 'FI_ACCOUNT', '7800.00','0.00', '108', '100'),
...
#Insertion into Table countries:
...
#Insertion into Table locations:
...
#Creation of Table regions:
CREATE TABLE IF NOT EXISTS `regions` ( `REGION_ID` decimal(5,0) NOT NULL,
`REGION_NAME` varchar(25) DEFAULT NULL, PRIMARY KEY (`REGION_ID`), UNIQUE KEY
`sss` (`REGION_NAME`));
#Insertion into Table regions:
...
#3
UPDATE employees SET JOB_ID= 'SH_CLERK' WHERE employee_id=118 AND department_id=30;
#4
...
#20
select first_name, last_name , length(first_name) + length(last_name) "Length of the name" from employees;
#21
...
#23
#24
select First_name, last_name from employees where sarlary >10000 and salary<15000;
#25
...
#30
SELECT first_name, last_name, hire_date, salary,  (DATEDIFF(now(), hire_date))/365 Experience  FROM departments d JOIN employees e 
ON (d.manager_id = e.employee_id) WHERE (DATEDIFF(now(), hire_date))/365>15;
